...
**HotSpot JVM**에서는 최적화 기법중 하나로 **JDK**의 일부 **정적 메소드**들에 대한 고성능의 **내장 함수(intrinsic function)**들을 제공한다. **JDK**의 개발자들은 **HotSpot**의 **내장 함수**로 치환되는 **정적 메소드**들을 **@HotSpotIntrinsicCandidate**으로 표시하도록 강제되었다. 표시를 할뿐, 실제로 **@HotSpotIntrinsicCandidate**를 통해 **javac**의 Annotation Processor**가 **내장 함수** 또는 **네이티브 코드**로 변환해주는 기능은 없다. 해당 기능은 **JIT**의 역할이기 때문이다.

### 1.2. HotSpot의 JIT에서 @HotSpotIntrinsicCandidate를 가진 메소드들만 *내장 함수*로 치환하도록 제한.
...
### 1.3. JDK 사용자가 HotSpot의 내장함수 기능을 최대한 활용 할 수 있게 함
